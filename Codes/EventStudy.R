#' ---
#' title: "Running Event Study using `fixest`"
#' author: "Taro Mieno"
#' output:
#'   html_document:
#'     number_sections: yes
...
#' # Source
#/*=================================================*/
#' The original code is found [here](https://lost-stats.github.io/Model_Estimation/Research_Design/event_study.html)
#' 
...
mod_twfe <- 
  feols(asmrs 
    ~ i(time_to_treat, treat, ref = -1) # Our key interaction: time × treatment status
...
mod_sa <- 
  feols(asmrs 
    ~ sunab(year_treated, year) + ## The only thing that's changed
    + pcinc + asmrh + cases 
    | stfips + year,
    cluster = ~stfips,
    data = dat
...
#' Again, we can use iplot() to depict the resulting event 
#' study in graphical form. In this case, we’ll plot both 
#' the new Sun-Abraham mode results and the previous (naive) 
#' TWFE model results

iplot(
  list(mod_twfe, mod_sa), 
  sep = 0.5, 
  ref.line = -1,
  xlab = 'Time to treatment',
...
legend(
  "bottomleft", col = c(1, 2), pch = c(20, 17), 
